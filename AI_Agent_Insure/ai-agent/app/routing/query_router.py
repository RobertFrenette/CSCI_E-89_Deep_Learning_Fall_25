...
def detect_query_type(question: str) -> tuple[str, str]:
    """
    Intelligently detect whether query is for SQL database, MongoDB, or RAG system.
    ...
    3. Keyword analysis (insureds, policies, claims vs user profiles vs company info)
    
    SQL queries about client/policyholder data are automatically detected.
    MongoDB queries about user profiles are automatically detected.
    ...
    Returns:
        ...
        query_type: 'sql', 'mongo', 'rag', or 'hybrid'
    ...
    # Check for MongoDB user profile indicators
    for indicator in MONGO_INDICATORS['strong']:
        if indicator in question_lower:
            mongo_score += SQL_STRONG_WEIGHT
            logger.debug(f"MongoDB indicator (strong): '{indicator}'")
    
    for indicator in MONGO_INDICATORS['operations']:
        if indicator in question_lower:
            mongo_score += SQL_MEDIUM_WEIGHT
            logger.debug(f"MongoDB indicator (operation): '{indicator}'")
    
    for indicator in MONGO_INDICATORS['qualifiers']:
        if indicator in question_lower:
            mongo_score += SQL_MEDIUM_WEIGHT
            logger.debug(f"MongoDB indicator (qualifier): '{indicator}'")
    ...
    # Decision logic
    logger.debug(f"Routing scores - SQL: {sql_score}, MongoDB: {mongo_score}, RAG: {rag_score}")
    
    # Check for MongoDB queries first (user profiles)
    if mongo_score >= SQL_MIN_SCORE_THRESHOLD and mongo_score >= sql_score:
        logger.info(f"MongoDB routing: score {mongo_score} vs SQL {sql_score} vs RAG {rag_score}")
        return 'mongo', question_stripped
    
    # Check for procedural queries (how-to questions) - prioritize RAG for these
    has_procedure_indicator = any(indicator in question_lower for indicator in RAG_INDICATORS['procedures'])
    if has_procedure_indicator and rag_score >= RAG_MEDIUM_WEIGHT:
        logger.info(f"RAG routing (procedure): SQL score {sql_score} vs RAG score {rag_score}")
        return 'rag', question_stripped
    
    # Check for hybrid queries (both SQL and RAG indicators present)
    if sql_score >= SQL_MIN_SCORE_THRESHOLD and rag_score >= RAG_MEDIUM_WEIGHT:
        logger.info(f"Hybrid routing: SQL score {sql_score}, RAG score {rag_score}")
        return 'hybrid', question_stripped
    elif sql_score > rag_score and sql_score >= SQL_MIN_SCORE_THRESHOLD:
